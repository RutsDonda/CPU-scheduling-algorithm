...
function addProcess() {
    ...
    row1.innerHTML = rowHTML1;
    row1.classList.add('row-animate');
    let row2 = table.insertRow(table.rows.length);
    row2.innerHTML = rowHTML2;
    row2.classList.add('row-animate');
    // Remove animation class after it completes
    setTimeout(() => {
        row1.classList.remove('row-animate');
        row2.classList.remove('row-animate');
    }, 600);
    checkPriorityCell();
    addremove();
    updateColspan();
...
function setAlgorithmNameType(input, algorithm) {
    ...
    if (algorithm === 'fcfs' || algorithm === 'sjf' || algorithm === 'pnp') {
...
function showFinalTable(input, output, outputDiv) {
    ...
    finalTableHeading.innerHTML = "<i class='fa fa-table' style='margin-right: 8px;'></i>Final Table";
    ...
    for (let i = 0; i < process; i++) {
        ...
        row.style.animation = `rowAppear 0.4s ease-out ${i * 0.08}s both`;
    ...
    if (input.contextSwitch > 0) {
        ...
        cs.innerHTML = "<i class='fa fa-exchange' style='margin-right: 6px; color: #6c63ff;'></i>Number of Context Switches : " + (output.contextSwitches - 1);
        outputDiv.appendChild(cs);
    }
    let avgTAT = output.turnAroundTime.reduce((acc, val) => acc + val, 0) / process;
    let avgWT = output.waitingTime.reduce((acc, val) => acc + val, 0) / process;

    let avgTATParagraph = document.createElement("p");
    avgTATParagraph.innerHTML = "<i class='fa fa-bar-chart' style='margin-right: 6px; color: #6c63ff;'></i>Average Turnaround Time: <strong>" + avgTAT.toFixed(2) + "</strong>";
    outputDiv.appendChild(avgTATParagraph);

    let avgWTParagraph = document.createElement("p");
    avgWTParagraph.innerHTML = "<i class='fa fa-hourglass-half' style='margin-right: 6px; color: #e94560;'></i>Average Waiting Time: <strong>" + avgWT.toFixed(2) + "</strong>";
    outputDiv.appendChild(avgWTParagraph);
}
...
function calculateOutput() {
    ...
    outputDiv.innerHTML = "";
    // Add reveal animation to output
    outputDiv.classList.remove('output-reveal');
    void outputDiv.offsetWidth; // Force reflow to restart animation
    outputDiv.classList.add('output-reveal');
